...
Public Class DAODepartamento
    ...
    Public Sub Guardar() Implements OpSql.Guardar
        ...
        Me.sql.CommandText = "INSERT INto departamentos values ('" & Departamento & "'," & Eliminado & ")"
    ...
    Public Sub Actualizar() Implements OpSql.Actualizar
        conn = Cnx.GetConection
        Me.sql.CommandText = "update departamentos set departamento='" & Departamento & "', eliminado=" & Eliminado & " where id=" & Id & ""
        Me.sql.Connection = conn
        'MsgBox(Me.sql.CommandText)
        Me.sql.ExecuteNonQuery()
    End Sub
    ...
    Public Function ListarTodos() As Object Implements OpSql.ListarTodos
        Dim consulta As New List(Of Departamento)
        Dim r As SqlDataReader
        Dim i As Integer = 0

        conn = Cnx.GetConection
        ' If  IsNot Nothing Then
        Me.sql.CommandText = "select * from departamentos where eliminado=" & Eliminado & " "
        Me.sql.Connection = conn
        ...
        If r.HasRows Then
            While r.Read()
                ...
                consulta.Add(New Departamento(r.GetValue((r.GetOrdinal("id"))), r.GetValue((r.GetOrdinal("departamento"))), r.GetValue((r.GetOrdinal("eliminado")))))
                i += 1
            End While
        End If
